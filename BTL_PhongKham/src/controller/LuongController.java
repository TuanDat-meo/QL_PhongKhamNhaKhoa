    ...
    public void timKiemLuong(String tuKhoa) {
        ...
        try (PreparedStatement preparedStatement = conn.prepareStatement(sql)) {
            ...
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                ...
                if (resultCount == 0) {
                    if (view != null) {
                        JOptionPane.showMessageDialog(view, "Không tìm thấy kết quả nào cho: '" + tuKhoa + "'", "Cảnh báo", JOptionPane.WARNING_MESSAGE);
                    }
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
            if (view != null) {
                JOptionPane.showMessageDialog(view, "Lỗi tìm kiếm dữ liệu lương: " + e.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
